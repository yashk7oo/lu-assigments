...
public class UsingPredicate {
	...
	public static void usingTwoArgsNewPredicate(CreditCard card,double expected,String name) {
		//creating a lambda for BIpreicate and its method test
		BiPredicate<Double,Double> checkLimit = (limit,expt)-> limit==expt;
		System.out.println(checkLimit.test(card.getCreditLimit(),expected));
		 
		BiPredicate<String,String> checkEquality = (getCardHolderName,newName) -> card.getCardHolderName().equals(name);
		System.out.println(checkEquality.test(card.getCardHolderName(),name));
	}
